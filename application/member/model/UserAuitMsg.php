...
class UserAuitMsg extends Model
{
   ...
   //通过会员id获取审核通知
   public function get_by_uid($user_id,$fields=['*'],$limit=1,$order=['id'=>'desc'])
   {
      return $this->field($fields)->where(['user_id'=>$user_id])->order($order)->limit($limit)->select();
   }
